...
In practical terms, we need to set up some experiments and record data. To obtain the internal temperature, a [Raspberry PicoPi Temperature Logger](https://github.com/LeonardWurmsdobler/PicoTemperatureLogger) records indoor temperature measurements. Outside temperatures are obtain from the [Cambridge Raw Daily Weather Data](https://www.cl.cam.ac.uk/weather/index-daily-text.html). We consider three periods:
...
Since we now have got a parameterised dynamic model, it should be possible to work out the dynamic heating requirements, too, and eventually the cost of heating. For this story, let's assume the January 2026 conditions with an average outdoor temperature T_o = 5°C. The heating requirement in a steady state would be Q_l = Q_r = 142.6 × 14 = **2.0 kW** to maintain an average of 19°C indoors, or ~48 kWh/day, to give a ballpark figure for the heating requirements. But houses are usually not maintained at a constant temperature throughout the day; this is where the dynamic simulation comes in.

## Gas Boiler Space Heating

Let's assume a traditional daily heating profile as pictured below using a gas boiler with a maximum power of 6kW(^1): heating off after 22h, heating on at 6h in the morning with a setpoint of T_s = 19°C and at full capacity, off at 9h when we leave the house but keep the house over as T_s = 15°C, on again at 17h until 22h with T_s = 19°C. The simplisitic relay-based thermostatic controller would do the following:

- if T_i < T_s, then full power until target temperature of T_s is reached; 
...
*Figure: January day heated with gas boiler, heat = 33.8 kWh/day, £3.01/day total household cost.*

The gas boiler can produce the required peak power of 5.3 kW and deliver the heat through the flow temperature reaching 74°C through the existing radiators. The simulation shows that with a daily heat delivery of 33.8 kWh, the boiler consumes 35.6 kWh of gas (at 95% efficiency). The total household daily cost is £3.01 (£2.11 gas energy + £0.35 gas standing charge + £0.55 electricity standing charge for household electricity needs).
...
With the power divided by the COP at each timestep, the total electricity consumption is **10.5 kWh/day**. At the January 2026 electricity price of 27.69 p/kWh plus 54.75p standing charge, this would cost **£3.44/day**. Compare this to the gas boiler household cost of £3.01/day, and we see that running a heat pump like a gas boiler on a flat tariff costs **14% more** than gas despite the favorable spark gap of 4.67.

![Heat Pump COP Profile](assets/heat_pump_cop_simulation.png)
*Figure: January day with equivalent heat pump, heat = 33.8 kWh/day, electricity = 10.5 kWh/day, £3.44/day total household cost.*
...
*Figure: Tariff-optimized heat pump operation, heat = 41.3 kWh/day, electricity = 10.0 kWh/day, £3.06/day total household cost.*
...
- **Gas boiler (traditional on/off control)**: £3.01/day  
  Heat: 33.8 kWh/day, Gas: 35.6 kWh/day, Cost: £2.11 gas energy + £0.35 gas SC + £0.55 elec SC

- **Heat pump with simple thermostat control (mimicking gas boiler operation)**: £3.44/day (+14% vs gas)  
  Heat: 33.8 kWh/day, Electricity: 10.5 kWh/day, SCOP: 3.23, Cost: £2.90 energy + £0.55 elec SC  
  Flow temperatures spike to 73°C, reducing efficiency during warm-up periods.

- **Heat pump with smooth continuous operation (flat tariff)**: £2.84/day (−6% vs gas)  
...
- **Heat pump with tariff-optimized operation (Octopus Cosy)**: £3.06/day (+2% vs gas)  
...
- DIY: set up [Home Assistant](https://www.home-assistant.io/), automate your home with sensors and actuators, and spend your weekends on optimisation of the home energy system;
...
1. **Boiler efficiency:** The Viessmann Vitodens 222-F condensing gas boiler achieves 95% efficiency under typical operating conditions and a rated power of 26kW; only a fraction is used for heating due to the radiator flow temperature limit.

2. **Energy costs**: as of January 2026, Ofgem energy price cap for a typical dual-fuel household paying by Direct Debit sets electricity at 27.69p per kWh with a 54.75p daily standing charge, and gas at 5.93p per kWh with a 35.09p daily standing charge. All cost comparisons in this analysis include electricity standing charges for all scenarios, reflecting the reality that all households pay for electricity grid connection regardless of heating method.

3. **Octopus Cosy**: This tariff is location dependent and used to be quite interesting, cosy rate down to 8p/kWh. Now the rates are as follows. Day rate: 33.28p / kWh, Cosy rate (04:00 - 07:00, 13:00 - 16:00, 22:00 - 00:00): 14.53p / kWh, Peak rate (16:00 - 19:00): 51.68p / kWh; Standing charge: 52.19p / day
