...
*Figure: For the current radiator constant K = 35.9 W/K^1.2, the flow temperature (left axis, blue) and the expected COP (right axis, green) plotted over heat power delivered. The red dashed line shows break-even COP = 3.6. The three marked points show: 1.96 kW with log burner (COP = 3.75, above break-even), 2.33 kW without log burner (COP = 3.48, slightly below), and 3.12 kW at peak condition (COP = 3.04, significantly below).*

### Scenario 1 Heat Pump With Log Burner

If the gas boiler is replaced with a heat pump whilst continuing to use the log burner for supplemental heat, the heat balance remains with radiators delivering Q_r = 1.96 kW, log burner providing Q_b = 0.375 kW, appliances contributing Q_a = 0.3 kW, and total heat Q_l = 2.635 kW (including internal gains). High flow operation optimised for the heat pump at 20 l/min requires a flow temperature of 48°C with return temperature ~46°C (ΔT ≈ 1.4K), delivering 1.96 kW heating power. The estimated COP is 3.75 (at T_o=5°C, T_f=48°C), requiring electrical input of ~0.52 kW for daily electricity consumption of 12.5 kWh and daily cost of £3.13 at 25p/kWh. 

With the log burner continuing to provide supplemental heat and appliances contributing internal gains, the heat pump operates well above the break-even threshold with comfortable 48°C flow temperature. Daily operating cost (£3.13) is 10% lower than gas (£3.47), making this scenario economically attractive whilst achieving fossil fuel independence. The COP of 3.75 is comfortably above the spark gap of 3.6.
...
Heat pump operation at maximum flow (20 l/min) requires a flow temperature of 52°C with return temperature ~51°C (ΔT ≈ 1.7K) to deliver 2.335 kW heating power. The estimated COP is 3.48 (at T_o=5°C, T_f=52°C), requiring electrical input of ~0.67 kW for daily electricity consumption of 16.1 kWh and daily cost of £4.03 at 25p/kWh. The operating cost is thus 16% higher than the gas+log burner configuration (£3.47/day), and the COP of 3.48 falls slightly below the break-even threshold of 3.6.

### Scenario 3 Heat Pump Without Log Burner at Peak Condition

The constraint is exacerbated during peak conditions. For a conservative design representing the theoretical full load during the coldest conditions, appliances contribute Q_a = 0.3 kW, leaving the heat pump to deliver 3.42 - 0.3 = 3.12 kW. At maximum flow (20 l/min), this requires a flow temperature of ~61°C with COP dropping to 3.04 and daily cost of £6.16. This worst-case scenario confirms the need for radiator upgrades: attempting to deliver peak heating loads with insufficient radiator capacity forces operation at 61°C flow temperature, pushing the COP below the break-even threshold of 3.6.
...
*Figure: For three heating power levels, the flow temperature (left axis, solid lines) and the expected COP (right axis, dashed lines) plotted over radiator constant K. The circles mark where COP reaches 3.6 (break-even threshold). Current K = 35.9 W/K^1.2 is already sufficient for 1.96 kW (blue), requires modest upgrade to 38.5 for 2.33 kW (green), and needs upgrade to 52 for 3.12 kW (red).*

The conclusion is nuanced. For the 1.96 kW scenario with log burner, the current radiators are sufficient with COP = 3.75 already above break-even. For the 2.335 kW scenario without log burner, a modest upgrade to K ≈ 38.5 W/K^1.2 (1.1× current) would raise COP from 3.48 to 3.6. For the 3.12 kW peak condition, a more substantial upgrade to K ≈ 52 W/K^1.2 (1.4× current) is required to achieve break-even COP. The upgrade transforms the economics and practical viability of heat pump operation without supplemental heating, enabling all scenarios to operate at or above the break-even threshold.

# Conclusion

Current radiators can support economically viable heat pump operation with supplemental heating (log burner), achieving COP = 3.75 and costs 10% below gas heating. However, without supplemental heating, performance falls below break-even for both average (COP = 3.48) and peak conditions (COP = 3.04). The recommended upgrade specification is: K value of 52 W/K^1.2 (approximately 1.4× current capacity), achieved by replacing existing radiators with larger Type 22 panels, adding additional radiators in key rooms, or upgrading to Type 33 (triple panel) in limited spaces. This would enable delivery of the 3.12 kW peak load at approximately 51°C flow temperature (accounting for 0.3 kW appliance contribution to reach theoretical 3.42 kW total), ensuring all scenarios operate at or above the break-even COP of 3.6. While not absolutely essential if retaining supplemental heating, radiator upgrades are strongly recommended to achieve economic independence from fossil fuels year-round.
