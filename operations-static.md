...
In an equilibrium, or steady state, the heat balance for the radiator circuit (no losses in short pipework), is Q_h = Q_r, and for the room thermal balance, Q_r + Q_b + Q_a = Q_l; the balance is invariant to the thermal mass C. Overall, the radiator system must deliver:
...
*Figure: For the current radiator constant K = 35.9 W/K^1.2, the flow temperature (left axis, blue) and the expected COP (right axis, green) plotted over heat power delivered. The red dashed line shows break-even COP = 4.67 (spark gap at January 2026 prices). The three marked points show: 1.96 kW with log burner (COP = 3.75, below break-even), 2.33 kW without log burner (COP = 3.48, further below), and 3.12 kW at peak condition (COP = 3.04, well below).*

### Scenario 1 Heat Pump With Log Burner

If the gas boiler is replaced with a heat pump whilst continuing to use the log burner for supplemental heat, the heat balance remains with radiators delivering Q_r = 1.96 kW, log burner providing Q_b = 0.375 kW, appliances contributing Q_a = 0.3 kW, and total heat Q_l = 2.635 kW (including internal gains). High flow operation optimised for the heat pump at 20 l/min requires a flow temperature of 48°C with return temperature ~46°C (ΔT ≈ 1.4K), delivering 1.96 kW heating power. The estimated COP is 3.75 (at T_o=5°C, T_f=48°C), requiring electrical input of ~0.52 kW for daily electricity consumption of 12.5 kWh and daily electricity cost of £3.47 at 27.69p/kWh. Adding the log burner wood cost of £1.00/day gives a total daily cost of £4.47.

For comparison, gas for space heating (49.6 kWh/day at 5.93p/kWh) costs £2.94/day; including the log burner at £1.00/day brings the current total to £3.94/day. The heat pump scenario therefore costs £0.53/day more, approximately 13% more expensive than the current gas-plus-log-burner setup. The COP of 3.75 falls below the spark gap of 4.67, indicating that at January 2026 energy prices the heat pump with log burner does not offer an economic advantage over gas, even though it would achieve fossil fuel independence for space heating.
...
Heat pump operation at maximum flow (20 l/min) requires a flow temperature of 52°C with return temperature ~51°C (ΔT ≈ 1.7K) to deliver 2.335 kW heating power. The estimated COP is 3.48 (at T_o=5°C, T_f=52°C), requiring electrical input of ~0.67 kW for daily electricity consumption of 16.1 kWh and daily cost of £4.46 at 27.69p/kWh. The operating cost is thus 13% higher than the current gas-plus-log-burner baseline (£3.94/day), and the COP of 3.48 falls below the break-even threshold of 4.67.

### Scenario 3 Heat Pump Without Log Burner at Peak Condition

The constraint is exacerbated during peak conditions. For a conservative design representing the theoretical full load during the coldest conditions, appliances contribute Q_a = 0.3 kW, leaving the heat pump to deliver 3.42 - 0.3 = 3.12 kW. At maximum flow (20 l/min), this requires a flow temperature of ~61°C with COP dropping to 3.04 and daily cost of £6.82 at 27.69p/kWh (24.6 kWh/day). This worst-case scenario underlines the case for radiator upgrades: attempting to deliver peak heating loads with insufficient radiator capacity requires operation at 61°C flow temperature, pushing the COP well below the break-even threshold of 4.67.


## Adjusting the Radiator Constant

At this point it is clear that radiators need to be upgraded, i.e. the constant K needs to be higher, but to what extent? To this end, let's work out the flow temperature and the expected COP plotted over a variable radiator constant K, but for various heating power: the base load with log burner (1.96kW), one without log burner (2.335 kW) and the peak condition (3.12 kW). The intersection with the minimum COP to break even should reveal the required K for each case. (Note, heat pump temperature needs to be 5°C above the radiator flow temperature).

![Performance vs K](assets/performance_vs_k.png)
*Figure: For three heating power levels, the flow temperature (left axis, solid lines) and the expected COP (right axis, dashed lines) plotted over radiator constant K. The circles mark where COP reaches 4.67 (break-even threshold at January 2026 prices). To achieve break-even, K must be upgraded to 64 for 1.96 kW (blue, 1.8× current), to 76 for 2.33 kW (green, 2.1× current), and to 104 for 3.12 kW (red, 2.9× current). All break-even points converge to the same flow temperature of 37.1°C.*

The spark gap of 4.67 raises the break-even threshold substantially, requiring low flow temperatures (37.1°C) to achieve economic parity with gas. At January 2026 energy prices, none of the scenarios reach break-even with the current radiator constant. For the 1.96 kW scenario with log burner, an upgrade to K ≈ 64 W/K^1.2 (1.8× current) is required. For the 2.335 kW scenario without log burner, K ≈ 76 W/K^1.2 (2.1× current) is needed. For the 3.12 kW peak condition, K ≈ 104 W/K^1.2 (2.9× current) is required. Notably, all three break-even points converge to the same 37.1°C flow temperature, which is the physical constraint imposed by the spark gap: heat pump operation is only economic when the flow temperature is low enough to yield COP ≥ 4.67.

# Conclusion

The recommended upgrade specification for economic break-even with fossil fuel independence is K ≈ 104 W/K^1.2 (approximately 2.9× current capacity, a substantial upgrade), achieved by replacing most radiators with larger Type 22 or Type 33 panels and adding radiators in key rooms. Even for the log-burner-supplemented scenario (1.96 kW), an upgrade to K ≈ 64 W/K^1.2 (1.8× current) is required to reach break-even. These upgrades are more demanding than a lower spark gap would imply, and reflect the current energy price environment where the motivation for switching would be decarbonisation rather than cost saving, unless a significant change in the electricity pricing (e.g. move a away from merit order based marginal pricing).
...
2. **Energy costs** as of January 2026 for British Gas standard tariff as the worst case: Natural gas at 5.93p/kWh and electricity assumed at 27.69p/kWh for heat pump analysis as an obtainable price depending on the tariff; a "spark gap" of 27.69/5.93 = 4.67.
